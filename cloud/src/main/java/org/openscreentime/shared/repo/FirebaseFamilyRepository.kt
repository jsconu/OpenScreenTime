...
class FirebaseFamilyRepository(
    ...
    override suspend fun signUpParent(email: String, password: String): String = session.signUpParent(email, password)

    override suspend fun signInParent(email: String, password: String): String = session.signInParent(email, password)

    /** Sends a password-reset email to an existing parent account (see [SessionRepository.sendPasswordReset]). */
    override suspend fun sendPasswordReset(email: String) = session.sendPasswordReset(email)

    /** For recovering a forgotten family passcode: proves the person knows the account password. */
    override suspend fun verifyAccountPassword(password: String): Boolean = session.verifyAccountPassword(password)

    override suspend fun signInAnonymously(): String = session.signInAnonymously()

    override fun signOut() = session.signOut()

    // --- Parent side ---

    override suspend fun createChild(parentUid: String, name: String): ChildProfile = children.createChild(parentUid, name)
    ...
    override suspend fun getParentPasscode(parentUid: String): PasscodeInfo? = passcodes.getParentPasscode(parentUid)
    ...
    override fun listenChildren(parentUid: String, onChange: (List<ChildProfile>) -> Unit): Registration =
        children.listenChildren(parentUid, onChange).asRegistration()
    ...
    ): Registration = stats.listenDailyStats(parentUid, childId, date, onChange).asRegistration()
    ...
    override suspend fun setAppListMember(parentUid: String, childId: String, list: AppList, packageName: String, member: Boolean) =
    ...
    override suspend fun setTrackingToggle(parentUid: String, childId: String, toggle: TrackingToggle, enabled: Boolean) =
    ...
    override suspend fun approveProposal(parentUid: String, childId: String, child: ChildProfile) =
        limits.approveProposal(parentUid, childId, child)

    /** Clears a pending proposal without applying it. See #14. */
    override suspend fun declineProposal(parentUid: String, childId: String) = limits.declineProposal(parentUid, childId)
     ...
     */
    override suspend fun requestExtraTime(parentUid: String, childId: String, minutes: Int) =
    ...
    override suspend fun declineExtraTimeRequest(parentUid: String, childId: String) =
    ...
    override suspend fun deleteChild(parentUid: String, childId: String) = children.deleteChild(parentUid, childId)

    // --- Kid side ---

    /** Resolves a pairing code and atomically links this device's anonymous uid to the child profile. */
    override suspend fun claimPairingCode(code: String): Pair<String, ChildProfile> = pairing.claimPairingCode(code)
     ...
     */
    override suspend fun getParentSelfProfile(parentUid: String): ChildProfile? = children.getParentSelfProfile(parentUid)

    override fun listenChild(parentUid: String, childId: String, onChange: (ChildProfile) -> Unit): Registration =
        children.listenChild(parentUid, childId, onChange).asRegistration()
    ...
    override suspend fun pushInstalledApps(parentUid: String, childId: String, apps: List<InstalledApp>) =
    ...
    ): Registration = stats.listenInstalledApps(parentUid, childId, onChange).asRegistration()

    /** See #22 - write-only; nobody can read feedback back through the app. */
    override suspend fun submitFeedback(parentUid: String, text: String, appVersion: String, device: String) =
        feedback.submit(parentUid, text, appVersion, device)
}

/** Firestore's own listener handle, as the transport-free [Registration] the apps see. */
private fun ListenerRegistration.asRegistration(): Registration {
    val listener = this
    return Registration { listener.remove() }
}
