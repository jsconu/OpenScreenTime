...
The parent's half: it tracks and limits the phone it is installed on, and - if a child's phone is linked by scanning a QR code - shows how that phone is being used and sets its limits, syncing directly between the two over the home Wi-Fi with no server involved.
...
`org.openscreentime.kid` — which are installed on different phones. The kid app is being requested separately; this one is useful on its own for an adult limiting their own phone.
